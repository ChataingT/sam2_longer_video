...
class VideoProcessor:
    ...
    def __init__(
        ...
        detect_interval=30,  # Perform detection every few frames (conditional frame prompt), -1 means no detection (no conditional frame prompt) but there must be at least one conditional frame in all frames (can be a system prompt frame)
        ...
        # Limit the number of frames in inference_state, and release old frames when exceeding this number, -1 means no limit
            ...
            while True:
                ...
                if not ret:  # End of video, perform inference on the remaining accumulated video frames in the buffer
                    if self.frame_buffer is not None and len(self.frame_buffer) > 0:
                        ...
                        self.Detect_and_SAM2_inference(frame_idx=self.pre_frames + frame_idx - 1)  # Index of the last frame, note that it needs to be -1 when the loop ends
                    break
...
if __name__ == '__main__':
    ...
    processor.run(
        video_path=rtsp_url,  # Provide video path (choose either this or frame folder)
